...
from LocalGlobalattention import *
...
class AONet:
    ...
    def initialize(self, cuda_device=None):
        ...
        self.model = LocalGlobalattention(input_size=1024,
                             output_size=1024,
                             num_segments=4,
                             heads=1,
                             fusion='add',
    ...
    def train(self, output_dir='EX-0'):
        ...
        for epoch in range(self.hps.epochs_max):
            ...
            path_feature_rgb = os.path.join(self.hps.kinetic_features,self.dataset_name,"RGB","features")
            path_feature_flow = os.path.join(self.hps.kinetic_features,self.dataset_name,"FLOW","features")
            # print(path_feature_flow)
            # print(path_feature_rgb)
            base_add_targets = os.path.join(self.hps.kinetic_features,self.dataset_name,"targets")
            ...
            for i, key in enumerate(train_keys):
                ...
                # print(f'{seq.shape} {target.shape}')
                # print(f'{type(seq)}{type(target)}')
                seq_len = seq1.shape[1]
                
                y, _ = self.model([seq1,seq2,seq3],seq_len)
                loss_att = 0
            ...
            val_fscore, video_scores = self.eval(self.test_keys,files_feature_rgb,files_feature_flow)
            ...
            max_val_fscorelist.append(max_val_fscore)

            if(len(max_val_fscorelist)>2 and max_val_fscorelist[-2]==max_val_fscorelist[-1]):
                samecount+=1
            else:
                samecount=0
            if self.verbose:
            ...
            filename = base_filename+'_'+str(self.split_id)+'_'+str(round(val_fscore*100,3))+'.tar.pth'
    ...
    def eval(self, keys, files_feature_rgb,files_feature_flow,video_name_plot,video_fscore_plot,results_filename=None):
        ...
        with torch.no_grad():
            for i, key in enumerate(keys):
                ...
                if(self.dataset_name=="tvsum"):
                    idx=self.getIdxInNames(files_feature_rgb,key)
                if(self.dataset_name=="summe"):
                    idx=self.getIdxInNamesSumme(files_feature_rgb,data['video_name'][...].astype(str))
                seq2 = np.load(files_feature_rgb[idx])
                seq3 = np.load(files_feature_flow[idx])
                ...
                minShape=np.min([seq1.shape[0],seq2.shape[0],seq3.shape[0]])
                # print(f'{seq1.shape} {seq2.shape} {seq3.shape} {minShape}')
                seq1 = cv2.resize(seq1, (seq1.shape[1],minShape), interpolation = cv2.INTER_AREA)
                seq2 = cv2.resize(seq2, (seq2.shape[1],minShape), interpolation = cv2.INTER_AREA)
                seq3 = cv2.resize(seq3, (seq3.shape[1],minShape), interpolation = cv2.INTER_AREA)

                # print(f'{seq1.shape} {seq2.shape} {seq3.shape} ')
                seq1 = torch.from_numpy(seq1).unsqueeze(0)
                seq2 = torch.from_numpy(seq2).unsqueeze(0)
                seq3= torch.from_numpy(seq3).unsqueeze(0)
        ...
        f_score, video_scores = self.eval_summary(summary, keys, metric=self.dataset_name,
                    results_filename=results_filename, att_vecs=att_vecs,video_name_plot=video_name_plot,video_fscore_plot=video_fscore_plot)

        return f_score, video_scores



    def eval_summary(self, machine_summary_activations, test_keys,video_name_plot,video_fscore_plot, results_filename=None, metric='tvsum', att_vecs=None):
        ...
        for key_idx, key in enumerate(test_keys):
            ...
            if metric=='tvsum':
                l=key.split('/')[-1]
                l=l.split('_')[-1]
                video_name_plot.append(l)
                video_fscore_plot.append(fm*100)
...
def eval_split(hps, splits_filename, data_dir='test'):
    ...
    path_feature_rgb = os.path.join(hps.kinetic_features,ao.dataset_name,"RGB","features")
    path_feature_flow = os.path.join(hps.kinetic_features,ao.dataset_name,"FLOW","features")
    base_add_targets = os.path.join(hps.kinetic_features,ao.dataset_name,"targets")
    files_feature_rgb  = glob.glob(os.path.join(path_feature_rgb,"*.npy"))
    files_feature_flow  = glob.glob(os.path.join(path_feature_flow,"*.npy"))
    files_feature_rgb.sort()
    files_feature_flow.sort()
    ...
    for split_id in range(len(ao.splits)):
        ...
        val_fscore, video_scores = ao.eval(ao.test_keys,files_feature_rgb,files_feature_flow,video_name_plot,video_fscore_plot)
    ...
    if ao.dataset_name=='tvsum':
        print(video_fscore_plot)
        fig, ax = plt.subplots()
        ax.plot(video_name_plot,video_fscore_plot)
        plt.xlabel('video number as per dataset TVSum')
        plt.ylabel('F-score')
        plt.show()
